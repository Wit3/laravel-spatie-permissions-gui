        ...
        <thead class="">
            <tr>
                <th class="relative hidden sm:table-cell">
                    <h3
                        class="absolute inset-0 items-end justify-start hidden pb-2 text-lg font-normal text-gray-600 sm:flex">
                        @lang("Permissions")</h3>
                </th>
                @if ($useActions)
                    @foreach ($actions as $key => $action)
                        <th class="p-1">
                            <div class="p-2 mb-1 text-xs font-normal rounded-md bg-cool-gray-100">
                                <i class='text-gray-700 pt-1 subpixel-antialiased text-base sm:text-2xl {{ $action }}'
                                    alt='{{ $key }}' style="min-width: 48px"></i>
                    ...
                    @if ($useActions)
                        @foreach ($actions as $key => $action)
                            ...
                            <td
                                ...
                                @else
                                    <i
                                        class='p-2 subpixel-antialiased font-bold text-orange-200 bg-gray-200 rounded-lg fas fa-exclamation-triangle text -lg cursor-disabled'></i>
                                @endif
                            </td>
                        @endforeach
                    @else
            ...
            @empty
                <tr>
                    <td class="w-full text-sm">
                        <div class="px-6 py-3 bg-yellow-100 border border-yellow-200 rounded">@lang('No permissions found.
                            Start with adding a new permission using the form below.')</div>
                    </td>
                </tr>
            @endforelse
        </tbody>
