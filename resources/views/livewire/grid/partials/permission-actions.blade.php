<div class="relative whitespace-no-wrap inline-block text-left w-full permission-ui-dropdown" x-data="{ open: false }" @keydown.window.escape="open = false"
     @click.away="open = false" @refresh-mappings.window="open = false">
    <div>
        <span class="rounded-md shadow-sm z-0">
            <button @click="open = !open" type="button"
                ...
                {{__(is_object($permission) ? $permission->name : $permission)}}
    ...
    <div x-show="open" x-cloak x-description="Dropdown panel, show/hide based on dropdown state."
         ...
         class="origin-top-left absolute left-0 mt-2 w-full rounded-md shadow-lg z-10 min-w-full"
        ...
        <div class="rounded-md bg-white" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
            ...
            <div class="px-3 py-3">
                ...
                <div class="w-full">
                    ...
                    <div class="bg-red-200 p-2 rounded text-gray-900 text-sm leading-4 mb-1"><b>@lang("Error"): </b>{{ $message }}</div>
                    @endforeach
                </div>
                @endif
                <div class="w-full mb-1">
                    <div class="text-xs">@lang("Permission Name") </div>
                ...
                <div class="w-full mb-1">
                    <button type="button" wire:click="updatePermission" href="#" class="w-full rounded p-1 px-3 text-center text-white bg-blue-500
                            hover:bg-blue-700 focus:outline-none" role="menuitem" wire:loading.attr="disabled">@lang("Update")
                    </button>
                </div>
                <div class="w-full mb-1">
                    <button type="button" wire:click="allowAll" href="#" class="w-full rounded p-1 px-3 text-center text-white bg-green-500
                    hover:bg-green-700 focus:outline-none" role="menuitem" wire:loading.attr="disabled">@lang("Authorize All")
                    </button>
                </div>
                <div class="w-full mb-1">
                    <button type="button" wire:click="disallowAll" href="#" class="w-full rounded p-1 px-3 text-center text-black bg-yellow-300
                    hover:bg-yellow-400 focus:outline-none" role="menuitem" wire:loading.attr="disabled">@lang("Unauthorize All")
                    </button>
                </div>
                <div class="w-full">
                    <button type="button" wire:click="deletePermission" href="#" class="w-full rounded p-1 px-3 text-center text-white bg-red-500
                    hover:bg-red-700 focus:outline-none" role="menuitem" wire:loading.attr="disabled">@lang("Delete")
                    </button>
                </div>
            </div>
        </div>
